<div>
    ...
    <div class="relative">
            ...
            <div class="h-10 flex items-center my-4">
                ...
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search" class="bg-base-100 w-full pl-10 py-3 text-sm leading-4 block rounded-md border-base-300 shadow-sm focus:border-base-300 focus:ring focus:ring-base-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('Search in')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
                        <div class="absolute inset-y-0 right-0 pr-2 flex items-center">
                            <button wire:click="$set('search', null)" class="text-base-content hover:text-red-600 focus:outline-none">
        ...
        <div wire:loading.class="opacity-50" class="rounded-lg @unless($complex || $this->hidePagination) rounded-b-none @endunless shadow-lg bg-base-100 max-w-screen overflow-x-scroll border-4 @if($this->activeFilters) border-blue-500 @else border-transparent @endif @if($complex) rounded-b-none border-b-0 @endif">
            <div>
                <div class="table align-middle min-w-full">
                    @unless($this->hideHeader)
                    <div class="table-row divide-x divide-base-300">
                        @foreach($this->columns as $index => $column)
                            @if($hideable === 'inline')
                    ...
                    @forelse($this->results as $row)
                        <div class="table-row p-1 {{ $this->rowClasses($row, $loop) }}">
                            @foreach($this->columns as $column)
                                ...
                                @else
                                    <div class="table-cell px-6 py-2 whitespace-nowrap align-middle @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @else text-left @endif {{ $this->cellClasses($row, $column) }}">
                                        {!! $row->{$column['name']} !!}
                                    </div>
                                @endif
                            @endforeach
        ...
        @unless($this->hidePagination)
            <div class="max-w-screen bg-base-100 @unless($complex) rounded-b-lg @endunless border-4 border-t-0 border-b-0 @if($this->activeFilters) border-blue-500 @else border-transparent @endif">
                <div class="p-2 sm:flex items-center justify-between">
                    {{-- check if there is any data --}}
                    @if(count($this->results))
                        <div class="my-2 sm:my-0 flex items-center">
                            <select name="perPage" class="mt-1 form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border-base-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5 bg-base-100 rounded" wire:model="perPage">
                        ...
                        <div class="flex justify-end text-base-content">
