<div wire:click.prefetch="toggle('{{ $index }}')"
    class="@if($column['hidden']) relative table-cell h-12 w-3 bg-base-100 hover:bg-base-300 overflow-none align-top group @else hidden @endif"
    style="min-width:12px; max-width:12px"
>
    <button class="relative h-12 w-3 focus:outline-none">
        <span
            class="w-32 hidden group-hover:inline-block absolute z-10 top-0 left-0 ml-3 bg-base-300 font-medium leading-4 text-xs text-left text-base-content tracking-wider transform uppercase focus:outline-none">
...
<div
    ...
    @if($column['unsortable'])
        <div class="w-full h-full px-6 py-3 border-b border-base-200 bg-base-50 text-xs leading-4 font-medium text-base-content uppercase tracking-wider flex justify-between items-center focus:outline-none">
    ...
    @else
        <button wire:click="sort('{{ $index }}')"
            class="w-full h-full px-6 py-3 border-b border-base-200 bg-base-50 text-xs leading-4 font-medium text-base-content uppercase tracking-wider flex justify-between items-center focus:outline-none">
            <span class="inline flex-grow @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @endif"">{{ str_replace('_', ' ', $column['label']) }}</span>
    ...
    @if ($column['hideable'])
        <button wire:click.prefetch="toggle('{{ $index }}')"
            ...
            <x-icons.arrow-circle-left class="h-3 w-3 text-base-300 hover:text-blue-400" />
        </button>
    @endif
</div>
